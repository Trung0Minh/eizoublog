        ...
        <div className="flex touch-pan-y">
          {POSTS.map((post) => (
            <div className="flex-[0_0_100%] min-w-0 relative aspect-[16/10] md:aspect-[21/9]" key={post.id}>
              <Link href={`/post/` + post.id} className="block w-full h-full relative cursor-pointer">
                <Image
                  src={post.coverImage}
                  alt={post.title}
                  fill
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent flex flex-col justify-end p-6 md:p-12">
                  <span className="bg-accent text-white text-[11px] font-bold uppercase tracking-wider px-3 py-1 rounded-full w-max mb-3 flex items-center gap-1 shadow-lg">
                    <Sparkles className="w-3 h-3" /> {post.category}
                  </span>
                  <h3 className="text-white text-[24px] md:text-[40px] font-display font-bold leading-tight mb-3 md:mb-4 max-w-3xl drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">
                    {post.title}
                  </h3>
                  <p className="text-white/80 text-[14px] md:text-[18px] font-serif max-w-2xl line-clamp-2 md:line-clamp-3 mb-4 drop-shadow-sm">
                    {post.excerpt}
                  </p>
                  <span className="text-white/60 text-[12px] md:text-[14px] font-medium">{post.date}</span>
                </div>
              </Link>
            </div>
          ))}
