# Nhập môn Sakuga Page Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a dedicated, editable static-style page "/nhap-mon-sakuga" (Nhập môn Sakuga) matching the design aesthetics and layout of the "/about" page, integrated with navbar/footer navigation links and the admin site-pages API.

**Architecture:** Create a Next.js App Router public page at `app/(public)/nhap-mon-sakuga/page.tsx` that fetches the page from the database and renders `IntroToSakugaClient.tsx` which provides the TipTap editor for admins and standard reading layout for other visitors.

**Tech Stack:** Next.js 16 (App Router), React 19, TipTap, Prisma, Tailwind CSS.

---

### Task 1: API Configuration for `nhap-mon-sakuga`

**Files:**
- Modify: `app/api/admin/site-pages/[slug]/route.ts`

- [ ] **Step 1: Update API route to support the new page slug**
  Modify lines 7-8 and 22-24 in `app/api/admin/site-pages/[slug]/route.ts` to include the `nhap-mon-sakuga` slug:
  ```typescript
  const editablePageSlugs = new Set(["about", "resources", "nhap-mon-sakuga"])
  ```
  And update `titleForSlug` to map it properly:
  ```typescript
  function titleForSlug(slug: string) {
    if (slug === "about") return "About"
    if (slug === "resources") return "Resources"
    if (slug === "nhap-mon-sakuga") return "Nhập môn Sakuga"
    return slug
  }
  ```
...
- [ ] **Step 3: Commit**
  ```bash
  git add app/api/admin/site-pages/\[slug\]/route.ts
  git commit -m "feat: enable nhap-mon-sakuga in admin site-pages API"
  ```
...
**Files:**
- Create: `app/(public)/nhap-mon-sakuga/page.tsx`

- [ ] **Step 1: Write the server component fetching from Prisma**
  Create the folder `app/(public)/nhap-mon-sakuga/` and write the server component `app/(public)/nhap-mon-sakuga/page.tsx`:
  ```tsx
...
- [ ] **Step 2: Verify file structure compiles**
  Run: `npx tsc --noEmit`
  Expected: Success (excluding the missing client component).

- [ ] **Step 3: Commit**
  ```bash
  git add 'app/(public)/nhap-mon-sakuga/page.tsx'
  git commit -m "feat: add Server Page component for /nhap-mon-sakuga"
  ```

---

### Task 3: Create Client Page Component with Default Expanded Content

**Files:**
- Create: `app/(public)/nhap-mon-sakuga/IntroToSakugaClient.tsx`

- [ ] **Step 1: Create client component with rich content**
  Create `app/(public)/nhap-mon-sakuga/IntroToSakugaClient.tsx` containing the Tiptap editor interface, saving logic, double-column layout matching the About page, and the default Vietnamese content:
  ```tsx
  "use client"
...
- [ ] **Step 2: Verify client page compiles**
  Run: `npx tsc --noEmit`
...
- [ ] **Step 3: Commit**
  ...
  git commit -m "feat: add client component with editor and default Vietnamese content"
  ```
...
- [ ] **Step 1: Add link to top Navigation bar**
  Add `{ href: "/nhap-mon-sakuga", label: "Nhập môn Sakuga" }` to `NAV_LINKS` inside `components/layout/Navbar.tsx`:
  ```typescript
  const NAV_LINKS = [
...
- [ ] **Step 2: Add link to Footer**
  Modify navigation links inside `components/layout/Footer.tsx` (around lines 26-52) to render the new page link:
  ```tsx
...
- [ ] **Step 3: Verify navigation layout compiles**
  Run: `npx tsc --noEmit`
  Expected: Success.
...
- [ ] **Step 4: Commit**
  ...
  git commit -m "style: link Nhập môn Sakuga in top Navbar and Footer navigation"
  ```
...
**Files:**
- Modify: `tests/unit/layout.test.tsx`

- [ ] **Step 1: Update layout assertions in test file**
  Update the expected navigation links in the unit tests to match the new `NAV_LINKS` list:
  ```typescript
  // Modify expected link arrays in tests/unit/layout.test.tsx to include /nhap-mon-sakuga
  ```

- [ ] **Step 2: Run layout unit tests**
...
- [ ] **Step 3: Commit**
  ...
  git commit -m "test: update layout unit tests with new navigation link"
  ```
